...
class CUB_200_2011(object):
    ...
    dataset_dir = "/data/cjl_data/CUB-200-2011/"
    def __init__(self,base):
        super(CUB_200_2011, self).__init__()
        self.train_dir = os.path.join(self.dataset_dir, 'train')
        self.val_dir = os.path.join(self.dataset_dir, 'val')
        self.test_dir = os.path.join(self.dataset_dir, 'test')
        wordpkl_path = os.path.join(self.dataset_dir, 'cub_bird_label_embeddings.pkl')#chang_ cub_bird_label_embeddings.pkl  vit-large-cub_bird_label_embeddings.pkl
        with open(wordpkl_path, 'rb') as f:  
            self.wordvecdir = pickle.load(f)  
        print(base)
        #if(False):
        #    self.wordvecdir=pca_manual(np.array(self.wordvecdir), 512)
        #    print("PCA OK")
        if(base=="C"):
