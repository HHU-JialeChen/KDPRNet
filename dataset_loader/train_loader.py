...
class FewShotDataset_train(Dataset):
    ...
    def _sample_episode(self):
        ...
        Tnovel = []
        Exemplars = []
        for Knovel_idx in range(len(Knovel)):  #变成0-4�?            
            ids = (nEvalExamplesPerClass + self.nExemplars)  #####75+1
    ...
    def _creatExamplesTensorData(self, examples,is_support=False):
        """
        ...
        if is_support:
            ...
            for (img_idx, label) in examples:
                img, ids, wordvec = self.dataset[img_idx]
                if self.load:
            ...
            for (img_idx, label) in examples:
                img, ids,_ = self.dataset[img_idx]
                if self.load:
                    img = Image.fromarray(img)
                else:
                    img = read_image(img)
        ...
        if is_support:
            ...
            return images, labels, cls, words
        else:
    ...
    def __getitem__(self, index):
        ...
        Xt, Yt, Ytc, tWords = self._creatExamplesTensorData(Exemplars,True)
        Xe, Ye, Yec = self._creatExamplesTensorData(Tnovel,False)
        return Xt, Yt, Xe, Ye, Ytc, Yec, tWords
