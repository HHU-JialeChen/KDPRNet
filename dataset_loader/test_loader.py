...
class FewShotDataset_test(Dataset):
    ...
    def _creatExamplesTensorData(self, examples,is_support=False):
        ...
        if is_support:
            ...
            for (img_idx, label) in examples:
                img,_, wordvec = self.dataset[img_idx]
                if self.load:
    ...
    def __getitem__(self, index):
        ...
        Xt, Yt, tWords = self._creatExamplesTensorData(Exemplars,True)
        Xe, Ye = self._creatExamplesTensorData(Tnovel,False)
        return Xt, Yt, Xe, Ye, tWords
